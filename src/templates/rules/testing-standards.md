...
- Commands that spawn external CLIs (such as `agy`, `claude`, `codex`, `opencode`, `agent`, `copilot`, `rtk`, or `graphify`) must accept an injectable `spawnFn` argument (defaulting to `node:child_process.spawnSync`).

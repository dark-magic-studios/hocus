import React, { useMemo, useState } from 'react';
import { Box, Text, useInput } from 'ink';
import fsExtra from 'fs-extra';
const { pathExists, remove } = fsExtra;
import path from 'node:path';
import { palette } from '../theme.js';
import { useDeck } from '../state/DeckContext.js';
import { useTextInput } from '../hooks/useTextInput.js';
import { useListWindow } from '../hooks/useListWindow.js';
import { installSkill } from '../../utils/files.js';
import { upgradeSingleSkill } from '../../utils/skill-audit.js';
import type { Skill } from '../state/types.js';

const SOURCE_COLOR: Record<string, string> = {
  bundled: palette.green,
  local: palette.muted,
  pack: palette.violet,
};

const SYNC_COLOR: Record<string, string> = {
  outdated: palette.amber,
  available: palette.violet,
  current: palette.green,
  local: palette.dim,
};

const SYNC_LABEL: Record<string, string> = {
  outdated: 'outdated',
  available: 'available',
  current: 'current',
  local: 'local',
};

type Mode = 'browse' | 'install-from';

export function GrimoireTab() {
  const { cwd, data, reload } = useDeck();
  const [mode, setMode] = useState<Mode>('browse');
  const [status, setStatus] = useState<string | undefined>();
  const [busy, setBusy] = useState(false);

  const outdatedCount = useMemo(
    () => data.skills.filter((s) => s.syncStatus === 'outdated' || s.syncStatus === 'available').length,
    [data.skills],
  );

  const reservedRows = 2 + (status ? 1 : 0) + (mode === 'install-from' ? 1 : 0);
  const {
    selectedIndex,
    windowStart,
    visibleEnd,
    move,
    showingLabel,
  } = useListWindow(data.skills.length, { reservedRows, linesPerItem: 2 });

  const selectedSkill: Skill | undefined = data.skills[selectedIndex];

  useInput(
    (input, key) => {
      if (mode !== 'browse' || busy) return;
      if (key.upArrow) move(-1);
      if (key.downArrow) move(1);
      if (input === ' ' && selectedSkill) void toggleMirror(selectedSkill);
      if (input === 'u' && selectedSkill) void updateSkill(selectedSkill);
      if (input === 'i') {
        setMode('install-from');
        setStatus(undefined);
      }
    },
    { isActive: mode === 'browse' && !busy },
  );

  const toggleMirror = async (skill: Skill) => {
    if (skill.syncStatus === 'outdated') {
      await updateSkill(skill);
      return;
    }
    if (skill.source === 'bundled' || skill.syncStatus === 'available') {
      const ok = await upgradeSingleSkill(cwd, skill.bundledId ?? skill.id);
      setStatus(ok ? `installed ${skill.name} into this project` : `install failed for ${skill.name}`);
    } else {
      await remove(path.join(cwd, '.claude', 'skills', skill.id)).catch(() => {});
      await remove(path.join(cwd, '.agents', 'skills', skill.id)).catch(() => {});
      const pluginsDir = path.join(cwd, '.agents', 'plugins');
      const plugins = await fsExtra.readdir(pluginsDir).catch(() => [] as string[]);
      for (const p of plugins) {
        await remove(path.join(pluginsDir, p, 'skills', skill.id)).catch(() => {});
      }
      setStatus(`removed ${skill.name} from this project`);
    }
    reload();
  };

  const updateSkill = async (skill: Skill) => {
    setBusy(true);
    try {
      const targetId = skill.bundledId ?? skill.id;
      const ok = await upgradeSingleSkill(cwd, targetId);
      if (!ok) {
        setStatus(`no bundled skill found for ${skill.name}`);
        return;
      }
      setStatus(`updated ${skill.name} from bundled pack`);
      reload();
    } catch (e) {
      setStatus(`update failed: ${String(e)}`);
    } finally {
      setBusy(false);
    }
  };

  const pathInput = useTextInput(
    mode === 'install-from',
    (value) => {
      void installFrom(value);
    },
    () => setMode('browse'),
  );

  const installFrom = async (fromPath: string) => {
    const trimmed = fromPath.trim();
    if (!trimmed) {
      setMode('browse');
      return;
    }
    if (!(await pathExists(trimmed))) {
      setStatus(`no skill found at ${trimmed}`);
      setMode('browse');
      return;
    }
    const name = path.basename(trimmed);
    await installSkill(trimmed, cwd, name);
    setStatus(`installed pack "${name}"`);
    setMode('browse');
    reload();
  };

  if (data.skills.length === 0) {
    return <Text color={palette.dim}>grimoire is empty. `hocus skill add &lt;name&gt;` to mirror one in.</Text>;
  }

  const visibleSkills = data.skills.slice(windowStart, visibleEnd);

  return (
    <Box flexDirection="column">
      <Text color={palette.dim}>
        {showingLabel}
        {outdatedCount > 0 ? (
          <>
            {' · '}
            <Text color={palette.amber}>{`${outdatedCount} need attention`}</Text>
          </>
        ) : null}
      </Text>

      {visibleSkills.map((s) => (
        <Box key={s.id} flexDirection="column">
          <Text>
            <Text color={s.id === selectedSkill?.id ? palette.green : palette.dim}>
              {s.id === selectedSkill?.id ? '▸ ' : '  '}
            </Text>
            <Text color={SOURCE_COLOR[s.source] ?? palette.muted}>{`[${s.source}]`.padEnd(11)}</Text>
            <Text color={SYNC_COLOR[s.syncStatus] ?? palette.dim}>
              {`[${SYNC_LABEL[s.syncStatus] ?? s.syncStatus}]`.padEnd(12)}
            </Text>
            <Text color={palette.text}>{s.name.padEnd(24)}</Text>
            <Text color={palette.dim}>{`${s.enabledFor.length} bound`}</Text>
          </Text>
          <Text color={palette.muted}>{`             ${s.description}`}</Text>
        </Box>
      ))}

      {mode === 'install-from' ? (
        <Text color={palette.violet}>install from path: {pathInput.value}<Text color={palette.dim}>▏</Text></Text>
      ) : null}

      {busy ? <Text color={palette.violet}>updating skill…</Text> : null}
      {status ? <Text color={palette.amber}>{status}</Text> : null}

      {data.warnings.filter((w) => w.includes('SKILL.md') || w.includes('skill')).map((w) => (
        <Text key={w} color={palette.amber}>{`! ${w}`}</Text>
      ))}
    </Box>
  );
}
